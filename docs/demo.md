...
3. Verify Bedrock access with the configured model before recording. Use the packet marked `STRANDS RUN`, which remains visually distinct from fixture packets.
...
4. **0:42-0:54 — Agent evidence.** Show the `STRANDS RUN` review packet and the four recorded scoped tool calls separately from the fixture packet.
...
- "A local Strands worker generated this cited draft through Amazon Bedrock."
