/* Shared behaviour for this app: theme, toasts, reveal motion, number helpers.
 * Class Catch-Up renders no charts, so the ECharts registry from the original
 * kit is stubbed out rather than bundled. */
const Kit = (() => {
  ...
  function theme(next) {
    if (next) {
      ...
      // are ES modules loaded independently of whoever calls this, and
  ...
  /* ── charts ────────────────────────────────────────────────────────────── */
  // A shared axis/grid style, so every chart on the dashboard reads the same.
  function base() {
    return {
  ...
  function chart() { console.warn('Kit.chart is not available in this build'); return null }
  function retheme() {}
  function update() {}
  function release() {}
